...
function generate_available_link_url($domain_id = 0) {
...
function get_full_url_from_link_url($url, $domain_id = 0) {
    $domain_id = (int) $domain_id;

    if($domain_id) {
        $domain = db()->where('domain_id', $domain_id)->getOne('domains', ['scheme', 'host', 'link_id']);
...
function create_qr_code_dynamic_link($user, $location_url, $name = '', $project_id = null, $url = null) {
    $url = $url ?: generate_available_link_url();
