...
class LinkAjax extends Controller {
    ...
    public function index() {
        ...
        register_shutdown_function([$this, 'log_fatal_ajax_error']);
        ...
        if(!empty($_POST) && (\Altum\Csrf::check('token') || \Altum\Csrf::check('global_token')) && isset($_POST['request_type'])) {
            ...
            try {
                ...
                $reference = $this->log_link_ajax_error('Unhandled LinkAjax exception', $exception);
                http_response_code(500);
                Response::json(l('global.error_message.basic') . ' Reference: ' . $reference, 'error', ['reference' => $reference]);
            }
    ...
    public function log_fatal_ajax_error() {
        $last_error = error_get_last();
        $fatal_error_types = [E_ERROR, E_CORE_ERROR, E_PARSE, E_COMPILE_ERROR];

        if(!$last_error || !in_array($last_error['type'], $fatal_error_types)) {
            return;
        }

        $reference = $this->log_link_ajax_error('Fatal LinkAjax error', null, $last_error);
        ...
        echo json_encode([
            'message' => [l('global.error_message.basic') . ' Reference: ' . $reference],
            'status' => 'error',
            'details' => ['reference' => $reference],
        ]);
    }

    private function log_link_ajax_error($label, \Throwable $exception = null, $fatal_error = null) {
        $reference = 'linkajax-' . date('Ymd-His') . '-' . bin2hex(random_bytes(4));
        ...
        $post = $_POST;
        foreach(['password', 'current_password', 'new_password', 'repeat_password', 'token', 'global_token'] as $sensitive_key) {
            if(isset($post[$sensitive_key])) {
                $post[$sensitive_key] = '[redacted]';
            }
        }
        ...
        if($fatal_error) {
            $entry['fatal_error'] = $fatal_error;
        }

        @file_put_contents($logs_directory . '/link-ajax-errors.log', json_encode($entry, JSON_PRETTY_PRINT) . PHP_EOL . PHP_EOL, FILE_APPEND | LOCK_EX);
        error_log('[LinkAjax] ' . $reference . ' ' . $label);
