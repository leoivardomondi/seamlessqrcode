...
class AdminPayments extends Controller {
    ...
    public function approve() {
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            $payment_datetime = $payment->datetime ?: get_date();
            $subscription_model = new \Altum\Models\Subscription();
            $renewal_period = $subscription_model->calculate_renewal_period($current_plan_expiration_date, $payment->frequency, $payment_datetime);
            $plan_expiration_date = $renewal_period['period_end'];
            ...
            try {
                $subscription_model->sync_from_successful_payment([
                    'local_payment_id' => $payment_id,
                    'external_payment_id' => $payment->payment_id,
                    'payment_processor' => $payment->processor ?: 'offline_payment',
                    ...
                    'period_start' => $renewal_period['period_start'],
                    'period_end' => $plan_expiration_date,
