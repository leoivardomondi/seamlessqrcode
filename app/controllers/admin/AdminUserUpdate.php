...
class AdminUserUpdate extends Controller {
    ...
    public function index() {
        ...
        $subscription_model = new \Altum\Models\Subscription();
        $subscription = $subscription_model->get_active_by_user_id($user->user_id);
        ...
        if(!empty($_POST)) {
            ...
            $_POST['type'] = isset($_POST['type']) && in_array((int) $_POST['type'], [0, 1, 2]) ? (int) $_POST['type'] : 0;
            ...
            $_POST['subscription_period_start'] = trim($_POST['subscription_period_start'] ?? '');
            $subscription_period_start = null;

            if($_POST['subscription_period_start']) {
                try {
                    $subscription_period_start = (new \DateTime(str_replace('T', ' ', $_POST['subscription_period_start'])))->format('Y-m-d H:i:s');
                } catch(\Exception $exception) {
                    Alerts::add_field_error('subscription_period_start', l('admin_user_update.plan.subscription_period_start_invalid'));
                }
            }
            ...
            if($user->user_id == \Altum\Authentication::$user_id && $_POST['type'] != 2) {
                Alerts::add_error('You cannot remove your own superadmin role.');
            }
            if($user->type == 2 && $_POST['type'] != 2 && db()->where('type', 2)->getValue('users', 'COUNT(*)') <= 1) {
                Alerts::add_error('At least one superadmin account is required.');
            }
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                try {
                    $subscription_sync_options = [];

                    if($subscription_period_start) {
                        $subscription_sync_options['period_start'] = $subscription_period_start;
                        $subscription_sync_options['event_datetime'] = $subscription_period_start;
                    }

                    $subscription_model->sync_from_user_plan($user->user_id, 'admin_user_update', $subscription_sync_options);
                } catch(\Exception $exception) {
                    if(DEBUG) {
