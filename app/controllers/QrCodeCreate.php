...
class QrCodeCreate extends Controller {
    ...
    public function index() {
        ...
        $default_frame = array_keys($frames)[1] ?? array_key_first($frames);
        ...
        if(!empty($_POST)) {
            ...
            $settings['size'] = $_POST['size'] = isset($_POST['size']) && in_array($_POST['size'], range(50, 2000)) ? (int) $_POST['size'] : 1864;
            $settings['margin'] = $_POST['margin'] = isset($_POST['margin']) && in_array($_POST['margin'], range(0, 25)) ? (int) $_POST['margin'] : 0;
            $settings['ecc'] = $_POST['ecc'] = isset($_POST['ecc']) && in_array($_POST['ecc'], ['L', 'M', 'Q', 'H']) ? $_POST['ecc'] : 'H';
            ...
            /* Frame */
            $settings['frame'] = $_POST['frame'] = isset($_POST['frame']) && array_key_exists($_POST['frame'], $frames) ? input_clean($_POST['frame']) : $default_frame;
            ...
            switch ($_POST['type']) {
                ...
                case 'url':
                    ...
                    $_POST['dynamic_url_slug'] = isset($_POST['dynamic_url_slug']) ? get_slug($_POST['dynamic_url_slug'], '-', false) : null;
            ...
            if($_POST['type'] == 'url' && isset($_POST['url_dynamic'])) {
                $destination_url = get_url($_POST['url']);
                ...
                if(!empty($_POST['link_id'])) {
                    $dynamic_link = db()->where('link_id', (int) $_POST['link_id'])->where('user_id', $this->user->user_id)->getOne('links', ['link_id', 'domain_id', 'url']);

                    if($dynamic_link) {
                        $settings['url'] = get_full_url_from_link_url($dynamic_link->url, $dynamic_link->domain_id);
                    } else {
                        $_POST['link_id'] = null;
                    }
                } else {
                    $dynamic_url_slug = $_POST['dynamic_url_slug'] ?: generate_available_link_url();

                    if(db()->where('url', $dynamic_url_slug)->where('domain_id', 0)->getValue('links', 'link_id')) {
                    ...
                    $_POST['dynamic_url_slug'] = $dynamic_url_slug;
                    $settings['url'] = get_full_url_from_link_url($dynamic_url_slug);
                }
            }
            ...
            if($_POST['is_bulk'] && $_POST['type'] == 'text') {
                ...
                if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                    if($_POST['type'] == 'url' && isset($_POST['url_dynamic'])) {
                        ...
                        if(!empty($_POST['link_id'])) {
                            db()->where('link_id', (int) $_POST['link_id'])->where('user_id', $this->user->user_id)->update('links', [
                                'location_url' => $destination_url,
                                'project_id' => $_POST['project_id'],
                                'last_datetime' => get_date(),
                            ]);
                            cache()->deleteItem('links?user_id=' . $this->user->user_id);
                            cache()->deleteItem('link?link_id=' . (int) $_POST['link_id']);
                            cache()->deleteItemsByTag('link_id=' . (int) $_POST['link_id']);
                        } else {
                            $_POST['link_id'] = create_qr_code_dynamic_link($this->user, $destination_url, $_POST['name'], $_POST['project_id'], $_POST['dynamic_url_slug']);
        ...
        $default_type = $_POST['type'] ?? $_GET['type'] ?? array_key_first($available_qr_codes);
        $default_url_dynamic = $_POST['url_dynamic'] ?? $_GET['url_dynamic'] ?? 1;

        $values = [
            'name' => $_POST['name'] ?? $_GET['name'] ?? '',
            'type' => $default_type,
            'project_id' => $_POST['project_id'] ?? $_GET['project_id'] ?? '',
            'url_dynamic' => $default_url_dynamic,
            'dynamic_url_slug' => $_POST['dynamic_url_slug'] ?? $_GET['dynamic_url_slug'] ?? generate_available_link_url(),
