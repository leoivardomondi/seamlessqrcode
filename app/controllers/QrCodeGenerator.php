...
class QrCodeGenerator extends Controller {
    ...
    public function index() {
        ...
        $_POST['size'] = isset($_POST['size']) && in_array($_POST['size'], range(50, 2000)) ? (int) $_POST['size'] : 1864;
        $_POST['margin'] = isset($_POST['margin']) && in_array($_POST['margin'], range(0, 25)) ? (int) $_POST['margin'] : 0;
        $_POST['ecc'] = isset($_POST['ecc']) && in_array($_POST['ecc'], ['L', 'M', 'Q', 'H']) ? $_POST['ecc'] : 'H';
        ...
        switch($_POST['type']) {
            ...
            case 'url':
                ...
                if(isset($_POST['url_dynamic'])) {
                    if(!empty($_POST['link_id'])) {
                        $link = db()->where('link_id', (int) $_POST['link_id'])->where('user_id', $user->user_id)->getOne('links', ['link_id', 'domain_id', 'url']);

                        if($link) {
                            $data = get_full_url_from_link_url($link->url, $link->domain_id);
                        }
                    } elseif(!empty($_POST['dynamic_url_slug'])) {
                        $dynamic_url_slug = get_slug($_POST['dynamic_url_slug'], '-', false);

                        if($dynamic_url_slug) {
                            $data = get_full_url_from_link_url($dynamic_url_slug);
                        }
                    }
                }
                break;
...
//                    }
        ...
        $frame = $_POST['frame'] = isset($_POST['frame']) && array_key_exists($_POST['frame'], $frames) ? input_clean($_POST['frame']) : $default_frame;
