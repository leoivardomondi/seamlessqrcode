...
class Links extends Controller {
    ...
    public function index() {
        ...
        while($row = $links_result->fetch_object()) {
            ...
            $row->qr_codes_count = 0;
            $links[] = $row;
        }

        if(count($links)) {
            $link_ids = array_map(fn($link) => (int) $link->link_id, $links);
            $qr_codes_counts = [];
            $qr_codes_counts_result = database()->query("
                SELECT `link_id`, COUNT(*) AS `total`
                FROM `qr_codes`
                WHERE `user_id` = {$this->user->user_id}
                    AND `link_id` IN (" . implode(',', $link_ids) . ")
                GROUP BY `link_id`
            ");

            while($row = $qr_codes_counts_result->fetch_object()) {
                $qr_codes_counts[$row->link_id] = (int) $row->total;
            }

            foreach($links as $row) {
                $row->qr_codes_count = $qr_codes_counts[$row->link_id] ?? 0;
            }
        }
