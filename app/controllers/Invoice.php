...
class Invoice extends Controller {
    ...
    public function proforma() {
        ...
        if((int) $invoice->user_id !== (int) $this->user->user_id) {
            redirect();
        }
    ...
    private function output_invoice_pdf($payment, array $payment_taxes = []) {
        ...
        $notes = $this->user->billing->notes ?? null;
        $pdf = (new \Altum\Helpers\InvoicePdf())->render($payment, $payment_taxes, ['notes' => $notes]);
