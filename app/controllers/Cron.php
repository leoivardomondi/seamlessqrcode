...
class Cron extends Controller {
    ...
    public function index() {
        ...
        $this->subscriptions_proforma_invoices();

        $this->subscriptions_dunning();
    ...
    private function subscriptions_dunning() {
        try {
            $processed = (new \Altum\Models\Subscription())->process_dunning();

            if(DEBUG && $processed) {
                echo sprintf('subscriptions_dunning() -> Processed %s invoice(s)', $processed);
            }
    ...
    private function subscriptions_proforma_invoices() {
        try {
            $processed = (new \Altum\Models\Subscription())->process_proforma_invoices();

            if(DEBUG && $processed) {
                echo sprintf('subscriptions_proforma_invoices() -> Created %s proforma invoice(s)', $processed);
            }
        } catch(\Exception $exception) {
            if(DEBUG) {
                echo $exception->getMessage();
