...
class QrCodeUpdate extends Controller {
    ...
    public function index() {
        ...
        $qr_code->dynamic_location_url = $qr_code->link_id && isset($links[$qr_code->link_id]) ? $links[$qr_code->link_id]->location_url : null;
        ...
        if(!empty($_POST)) {
            ...
            $settings['size'] = $_POST['size'] = isset($_POST['size']) && in_array($_POST['size'], range(50, 2000)) ? (int) $_POST['size'] : ($qr_code->settings->size ?? 1864);
            $settings['margin'] = $_POST['margin'] = isset($_POST['margin']) && in_array($_POST['margin'], range(0, 25)) ? (int) $_POST['margin'] : ($qr_code->settings->margin ?? 0);
            $settings['ecc'] = $_POST['ecc'] = isset($_POST['ecc']) && in_array($_POST['ecc'], ['L', 'M', 'Q', 'H']) ? $_POST['ecc'] : ($qr_code->settings->ecc ?? 'H');
