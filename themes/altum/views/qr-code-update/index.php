...
<div class="container">
    ...
    <div class="d-print-none">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex align-items-center">
                <?php if($data->qr_code->link_id): ?>
                    <a href="<?= url('qr-code-statistics/' . $data->qr_code->qr_code_id) ?>" class="btn btn-link text-secondary" data-toggle="tooltip" title="<?= l('link.statistics.link') ?>">
                        <i class="fas fa-fw fa-sm fa-chart-bar"></i>
                    </a>
                <?php endif ?>

                <?= include_view(THEME_PATH . 'views/qr-codes/qr_code_dropdown_button.php', ['id' => $data->qr_code->qr_code_id, 'link_id' => $data->qr_code->link_id, 'resource_name' => $data->qr_code->name]) ?>
            </div>
        </div>
    </div>
    ...
    <form id="form" action="" method="post" role="form" enctype="multipart/form-data">
        ...
        <input type="hidden" name="dynamic_url_slug" value="<?= generate_available_link_url() ?>" data-reload-qr-code />
        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        ...
                        <div>
                            <div class="form-group" data-type="url" data-url>
                                <label for="url"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('qr_codes.input.scan_destination') ?></label>
                                <input type="url" id="url" name="url" class="form-control <?= \Altum\Alerts::has_field_errors('url') ? 'is-invalid' : null ?>" value="<?= $data->qr_code->dynamic_location_url ?? $data->qr_code->settings->url ?? null ?>" maxlength="<?= $data->available_qr_codes['url']['max_length'] ?>" required="required" placeholder="<?= l('global.url_placeholder') ?>" data-reload-qr-code />
                                <small class="form-text text-muted"><?= l('qr_codes.input.scan_destination_help') ?></small>
                                <?= \Altum\Alerts::output_field_error('url') ?>
                            </div>

                            <div class="form-group" data-type="url" data-link-id>
                                <?php if($data->qr_code->link_id && isset($data->links[$data->qr_code->link_id])): ?>
                                    <?php $dynamic_link = $data->links[$data->qr_code->link_id]; ?>
                                    <input type="hidden" name="link_id" value="<?= $dynamic_link->link_id ?>" data-reload-qr-code />

                                    <label for="dynamic_short_link"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('qr_codes.input.dynamic_short_link') ?></label>
                                    <div class="input-group">
                                        <input id="dynamic_short_link" type="text" class="form-control" value="<?= $dynamic_link->full_url ?>" readonly="readonly" />
                                        <div class="input-group-append">
                                            <button
                                                    ...
                                                    data-clipboard-text="<?= $dynamic_link->full_url ?>"
                                            >
                                                <i class="fas fa-fw fa-sm fa-copy"></i>
                                            </button>
                                            <a href="<?= url('link/' . $dynamic_link->link_id) ?>" class="btn btn-outline-secondary" data-toggle="tooltip" title="<?= l('qr_codes.input.dynamic_short_link_edit') ?>">
                                                <i class="fas fa-fw fa-sm fa-pencil-alt"></i>
                                            </a>
                                        </div>
                                    </div>
                                    <small class="form-text text-muted"><?= l('qr_codes.input.dynamic_short_link_help') ?></small>
                                <?php else: ?>
                                    <div class="d-flex flex-column flex-xl-row justify-content-between">
                                    ...
                                    <select id="link_id" name="link_id" class="custom-select" data-reload-qr-code>
                                        <option value="" data-url="" data-location-url=""><?= l('global.none') ?> - <?= l('qr_codes.input.link_id_auto_create') ?></option>
                                        <?php foreach($data->links as $row): ?>
                                            <option value="<?= $row->link_id ?>" data-url="<?= $row->full_url ?>" data-location-url="<?= $row->location_url ?>">
                        ...
                        <div class="collapse" id="options_container" data-parent="#form">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <input id="size" type="number" min="50" max="2000" name="size" class="form-control <?= \Altum\Alerts::has_field_errors('size') ? 'is-invalid' : null ?>" value="<?= $data->qr_code->settings->size ?? 1864 ?>" data-reload-qr-code />
                                    <div class="input-group-append">
                            ...
                            <div class="form-group">
                                ...
                                <input id="margin" type="number" min="0" max="25" name="margin" class="form-control <?= \Altum\Alerts::has_field_errors('margin') ? 'is-invalid' : null ?>" value="<?= $data->qr_code->settings->margin ?? 0 ?>" data-reload-qr-code />
                            ...
                            <div class="form-group">
                                ...
                                <select id="ecc" name="ecc" class="custom-select" data-reload-qr-code>
                                    <?php foreach(['L', 'M', 'Q', 'H'] as $level): ?>
                                        <option value="<?= $level ?>" <?= ($data->qr_code->settings->ecc ?? 'H') == $level ? 'selected="selected"' : null ?>><?= l('qr_codes.input.ecc_' . mb_strtolower($level)) ?></option>
                                    <?php endforeach ?>
                                </select>
                            </div>
            ...
            <div class="col-12 col-lg-6">
                <div class="sticky">
                    ...
                    <button id="embedded_data_container_button" class="btn btn-block btn-light my-4 d-print-none" type="button" data-toggle="collapse" data-target="#embedded_data_container" aria-expanded="true" aria-controls="embedded_data_container">
                        <i class="fas fa-fw fa-bars fa-sm mr-1"></i> <?= l('qr_codes.embedded_data') ?>
                    </button>

                    <div class="collapse show" id="embedded_data_container">
                        <div class="card my-4">
                            <div class="card-body d-flex align-items-center justify-content-between">
                                <div id="embedded_data_display" class="text-break mr-3"><?= $data->qr_code->embedded_data ?></div>
                                <button
                                        id="embedded_data_copy"
                                        type="button"
                                        class="btn btn-link text-secondary flex-shrink-0"
                                        data-toggle="tooltip"
                                        title="<?= l('global.clipboard_copy') ?>"
                                        aria-label="<?= l('global.clipboard_copy') ?>"
                                        data-copy="<?= l('global.clipboard_copy') ?>"
                                        data-copied="<?= l('global.clipboard_copied') ?>"
                                        data-clipboard-text="<?= $data->qr_code->embedded_data ?>"
                                >
                                    <i class="fas fa-fw fa-sm fa-copy"></i>
                                </button>
                            </div>
