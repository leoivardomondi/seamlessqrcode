        ...
        <div class="collapse navbar-collapse justify-content-end" id="main_navbar">
            <ul class="navbar-nav">
                ...
                <?php foreach($data->pages as $data): ?>
                    ...
                    if(mb_strtolower(trim(strip_tags($data->title))) == 'pricing' || preg_match('/\/pricing\/?$/', parse_url($data->url, PHP_URL_PATH) ?? '')) {
                        $page_url = 'https://app.seamlessqrcode.com/plan';
                    }
                    ?>
                    <li class="nav-item">
                        <a class="nav-link" href="<?= $page_url ?>" target="<?= $data->target ?>">
                            <?php if($data->icon): ?>
...
<style>
    @media (min-width: 992px) {
        ...
        .navbar-main .dropdown-toggle .navbar-avatar + * {
            min-width: 0;
        }
    }
