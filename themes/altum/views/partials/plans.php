...
<?php if(settings()->payment->is_enabled): ?>
    ...
    <?php if(is_logged_in() && !is_numeric($this->user->plan_id ?? null)): ?>
        <?= include_view(THEME_PATH . 'views/partials/setup_planner.php') ?>
    <?php endif ?>

    <?php
    $available_payment_frequencies = [];

    /* Use controller-filtered plans when provided. */
    $plans = $data->plans ?? $plans ?? (new \Altum\Models\Plan())->get_plans();
    ...
    <?php if (!empty($plans)): ?>
        ...
        <div class="mb-5 text-center">
            <div class="btn-group btn-group-toggle btn-group-custom" data-toggle="buttons">
                ...
                $selected_payment_frequency = \Altum\Router::$controller == 'Lifetime' ? 'lifetime' : settings()->payment->default_payment_frequency;
                ?>

                <?php foreach($frequencies as $frequency): ?>
                    <?php if(isset($available_payment_frequencies[$frequency])): ?>
                        <label class="btn <?= $selected_payment_frequency == $frequency ? 'active' : null ?>" data-payment-frequency="<?= $frequency ?>">
                            <input type="radio" name="payment_frequency" <?= $selected_payment_frequency == $frequency ? 'checked="checked"' : null ?>> <?= l('plan.custom_plan.' . $frequency) ?>
                        </label>
                    <?php endif ?>
                <?php endforeach ?>
...
<div>
    <?php $default_plan_colors = ['#2563eb', '#059669', '#dc2626', '#7c3aed']; ?>
    <div class="row justify-content-around">

        <?php if(\Altum\Router::$controller != 'Lifetime' && settings()->plan_free->status == 1): ?>
            <?php $plan_free_color = settings()->plan_free->color ?: $default_plan_colors[0]; ?>

            <div class="col-12 col-lg-6 col-xl-4 p-3">
                <div class="pricing-plan position-relative rounded-2x" style="border-width: 3px; border-color: <?= $plan_free_color ?>">
                    ...
                    <div class="pricing-header">
                        <p class="pricing-name" style="color: <?= $plan_free_color ?>"><?= (settings()->plan_free->translations->{\Altum\Language::$name}->name ?? '') ?: settings()->plan_free->name ?></p>
                    ...
                    <div class="pricing-body d-flex flex-column justify-content-between">
                        <?= include_view(THEME_PATH . 'views/partials/plans_plan_content.php', [
                            'plan_settings' => settings()->plan_free->settings,
                            'plan_name' => settings()->plan_free->name,
                            'plan_id' => 'free',
                        ]) ?>

                        <?php if(settings()->users->register_is_enabled || is_logged_in()): ?>
                            <a href="<?= url('register') ?>" class="btn btn-block btn-primary rounded-2x <?= is_logged_in() && $this->user->plan_id != 'free' ? 'disabled' : null ?>" style="background-color: <?= $plan_free_color ?>; border-color: <?= $plan_free_color ?>"><?= l('plans.choose') ?></a>
                        <?php endif ?>
                    </div>
                </div>
            </div>
        ...
        <?php if(settings()->payment->is_enabled): ?>
            ...
            <?php $paid_plan_index = 1; ?>
            <?php foreach($plans as $plan): ?>
            <?php if($plan->status != 1) continue; ?>
            <?php $plan_color = $plan->color ?: $default_plan_colors[$paid_plan_index++ % count($default_plan_colors)]; ?>
            ...
            >
                <div class="pricing-plan position-relative rounded-2x" style="border-width: 3px; border-color: <?= $plan_color ?>">
                    ...
                    <div class="pricing-header">
                        <div>
                            <p class="pricing-name" style="color: <?= $plan_color ?>"><?= ($plan->translations->{\Altum\Language::$name}->name ?? '') ?: $plan->name ?></p>
                    ...
                    <div class="pricing-body d-flex flex-column justify-content-between">
                        <?= include_view(THEME_PATH . 'views/partials/plans_plan_content.php', [
                            ...
                            'plan_name' => ($plan->translations->{\Altum\Language::$name}->name ?? '') ?: $plan->name,
                            'plan_id' => $plan->plan_id,
                        ]) ?>

                        <?php if(settings()->users->register_is_enabled || is_logged_in()): ?>
                            <a href="<?= \Altum\Router::$controller == 'Lifetime' ? url('lifetime-checkout/' . $plan->plan_id) : url('register?redirect=pay/' . $plan->plan_id) ?>" class="btn btn-block btn-primary rounded-2x <?= is_logged_in() && $this->user->plan_id == $plan->plan_id && (new \DateTime($this->user->plan_expiration_date)) > (new \DateTime())->modify('+10 years') ? 'disabled' : null ?>" style="background-color: <?= $plan_color ?>; border-color: <?= $plan_color ?>">
                                <?php if(is_logged_in()): ?>
        ...
        <?php if(\Altum\Router::$controller != 'Lifetime' && settings()->plan_custom->status == 1): ?>
            <?php $plan_custom_color = settings()->plan_custom->color ?: $default_plan_colors[3]; ?>

            <div class="col-12 col-lg-6 col-xl-4 p-3">
                <div class="pricing-plan position-relative rounded-2x" style="border-width: 3px; border-color: <?= $plan_custom_color ?>">
                    ...
                    <div class="pricing-header">
                        <p class="pricing-name" style="color: <?= $plan_custom_color ?>"><?= (settings()->plan_custom->translations->{\Altum\Language::$name}->name ?? '') ?: settings()->plan_custom->name ?></p>
                    ...
                    <div class="pricing-body d-flex flex-column justify-content-between">
                        <?= include_view(THEME_PATH . 'views/partials/plans_plan_content.php', [
                            'plan_settings' => settings()->plan_custom->settings,
                            'plan_name' => settings()->plan_custom->name,
                            'plan_id' => 'custom',
                        ]) ?>

                        <?php if(settings()->users->register_is_enabled || is_logged_in()): ?>
                            <a href="<?= settings()->plan_custom->custom_button_url ?>" class="btn btn-block btn-primary rounded-2x" style="background-color: <?= $plan_custom_color ?>; border-color: <?= $plan_custom_color ?>"><?= l('plans.contact') ?></a>
                        <?php endif ?>
                    </div>
                </div>
            </div>
...
if(\Altum\Router::$controller != 'Lifetime' && (settings()->plan_guest->status ?? null)) {
    $offers[] = [
        '@type' => 'Offer',
...
if(\Altum\Router::$controller != 'Lifetime' && settings()->plan_free->status) {
    $offers[] = [
        '@type' => 'Offer',
...
if(\Altum\Router::$controller != 'Lifetime' && settings()->plan_custom->status) {
    $offers[] = [
        '@type' => 'Offer',
        ...
        foreach(['monthly', 'quarterly', 'biannual', 'annual', 'lifetime'] as $value) {
            if($plan->prices->{$value}->{currency()}) {
                $offers[] = [
                    ...
                    'availability' => 'https://schema.org/InStock',
                    'url' => \Altum\Router::$controller == 'Lifetime' ? url('lifetime-checkout/' . $plan->plan_id) : url('pay/' . $plan->plan_id)
                ];
            }
        }
    }
