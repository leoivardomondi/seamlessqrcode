...
$selected_payment_processor = $this->payment_processor
    && isset(settings()->{$this->payment_processor})
    && settings()->{$this->payment_processor}->is_enabled
    && in_array(currency(), settings()->{$this->payment_processor}->currencies ?? [])
        ? $this->payment_processor
        : ($selected_currency->default_payment_processor ?? null);
$selected_payment_type = isset($_GET['payment_type']) && in_array($_GET['payment_type'], ['one_time', 'recurring'])
    ? $_GET['payment_type']
    : (settings()->payment->default_payment_type ?? 'one_time');
?>
                ...
                <div class="col-12 col-xl-8">
                    ...
                    <div>
                        <div class="row d-flex align-items-stretch mx-n2">
                            ...
                            $wanted = isset($_GET['payment_frequency']) && in_array($_GET['payment_frequency'], ['monthly', 'quarterly', 'biannual', 'annual', 'lifetime']) ? $_GET['payment_frequency'] : (settings()->payment->default_payment_frequency ?? 'monthly');
                                                ...
                                                <div class="d-flex align-items-center justify-content-end">
                                                    <?php if($quarterly_price_savings > 0): ?>
                                                        <div id="quarterly_price_savings" class="payment-price-savings mr-2">
                                                            <span class="badge badge-pill badge-success" data-toggle="tooltip" title="<?= sprintf(l('global.plan_settings.quarterly_price_savings'), $quarterly_price_savings . ' ' . currency()) ?>">
                                ...
                                <?php foreach($data->payment_processors as $key => $value): ?>
                                    <?php if(settings()->{$key}->is_enabled && in_array(currency(), settings()->{$key}->currencies ?? [])): ?>
                                        <label class="<?= $enabled_payment_processors <= 4 ? 'col-12' : 'col-6' ?> p-2 custom-radio-box m-0">
                                            <input type="radio" name="payment_processor" value="<?= $key ?>" class="custom-control-input" required="required" <?= $key == $selected_payment_processor ? 'checked="checked"' : null ?>>
...
<script>
    ...
    let apply_default_payment_type = () => {
        let default_payment_type = <?= json_encode($selected_payment_type) ?>;
