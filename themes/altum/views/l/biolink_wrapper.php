...
<html lang="<?= \Altum\Language::$default_code ?>" class="link-html" dir="<?= l('direction') ?>">
    <head>
        ...
        <?php if(\Altum\Plugin::is_active('pwa') && settings()->pwa->is_enabled && $this->user->plan_settings->custom_pwa_is_enabled && $this->link->settings->pwa_is_enabled && !empty($this->link->settings->pwa_file_name)): ?>
        ...
        <?php else: ?>
            <?php require THEME_PATH . 'views/partials/pwa_head.php' ?>
        <?php endif ?>
    ...
    <?php if(!$this->is_preview): ?>
        ...
        <?php if(
                ...
                && file_exists(\Altum\Plugin::get('pwa')->path . 'views/partials/pwa_custom.php')
