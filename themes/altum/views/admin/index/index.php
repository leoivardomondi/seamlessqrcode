...
<?php $is_superadmin = \Altum\Authentication::is_superadmin(); ?>

<div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><?= sprintf(l('admin_index.header'), $this->user->name) ?></h1>

    <form action="<?= url('admin/index/clear-cache') ?>" method="post" role="form">
        <input type="hidden" name="token" value="<?= \Altum\Csrf::get() ?>" />
        <button type="submit" name="submit" class="btn btn-light">
            <i class="fas fa-fw fa-sm fa-database mr-1"></i> Clear cache
        </button>
    </form>
</div>
        ...
        <div class="col-12 col-sm-6 col-xl-3 p-3 position-relative">
            <div class="card d-flex flex-row h-100 overflow-hidden">
                <div class="card-body">
                    <div class="row">
                        <div class="col text-truncate">
                            <small class="text-muted font-weight-bold"><?= l('admin_redeemed_codes.menu') ?></small>
                        </div>

                        <div class="col-auto">
                            <span class="p-2 bg-primary-100 rounded">
                                <i class="fas fa-fw fa-sm fa-tags text-primary"></i>
                            </span>
                        </div>
                    </div>

                    <div class="mt-2 text-break">
                        <a href="<?= url('admin/redeemed-codes') ?>" class="stretched-link text-reset text-decoration-none">
                            <span class="h4" id="redeemed_codes">
                                <span class="spinner-border spinner-border-sm" role="status"></span>
                            </span>
                        </a>

                        <div class="mt-1 small text-muted">
                            <span id="redeemed_codes_current_month">
                                <span class="spinner-border spinner-border-sm" role="status"></span>
                            </span>
                            <?= mb_strtolower(l('global.date.this_month')) ?>
                        </div>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex">
                            ...
                            <div class="d-flex flex-column">
                                <div>
                                    <a href="<?= url('admin/user-view/' . $row->user_id) ?>" <?= $row->type > 0 ? 'class="font-weight-bold" data-toggle="tooltip" title="' . \Altum\Authentication::get_role_label($row) . '"' : null ?>><?= $row->name ?></a>
                                </div>
                    ...
                    <td class="text-nowrap">
                        <div class="d-flex flex-column">
                            <div>
                                ...
                                <?php else: ?>
                                    <span class="badge badge-light"><?= $data->plans[$row->plan_id]->name ?></span>
                                <?php endif ?>
                            </div>
...
<?php if($is_superadmin): ?>
    <?php $result = database()->query("
        SELECT
            ...
            `codes`.`plans_ids` AS `code_plans_ids`
        FROM
            `redeemed_codes`
        LEFT JOIN
        ...
        ORDER BY
            `redeemed_codes`.`id` DESC
        LIMIT 5
    "); ?>

    <?php if($result->num_rows): ?>
        <div class="mb-5">
            <h1 class="h3 mb-4"><i class="fas fa-fw fa-xs fa-tags text-primary-900 mr-2"></i> <?= l('admin_redeemed_codes.header') ?></h1>
            ...
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    ...
                    <?php while($row = $result->fetch_object()): ?>
                        ...
                        <?php
                        $code_plans_ids = json_decode($row->code_plans_ids ?? '[]');
                        $code_plans_ids = is_array($code_plans_ids) ? $code_plans_ids : [];
                        ?>
                        ...
                        <tr>
                            ...
                            <td class="text-nowrap">
                                <?php if($code_plans_ids): ?>
                                    <div class="d-flex flex-column align-items-start">
                                        <?php foreach($code_plans_ids as $plan_id): ?>
                                            <?php if(!isset($data->plans[$plan_id])) $data->plans[$plan_id] = (new \Altum\Models\Plan())->get_plan_by_id($plan_id) ?>
                                            <?php if(isset($data->plans[$plan_id]) && $data->plans[$plan_id]): ?>
                                                <a href="<?= url('admin/plan-update/' . $plan_id) ?>" class="badge badge-light mb-1"><?= $data->plans[$plan_id]->name ?></a>
                                            <?php else: ?>
                                                <span class="badge badge-light mb-1"><?= l('global.unknown') ?></span>
                                            <?php endif ?>
                                        <?php endforeach ?>
                                    </div>
                                <?php else: ?>
                                    <span class="badge badge-light"><?= l('global.unknown') ?></span>
                                <?php endif ?>
                            </td>

                            <td class="text-nowrap">
                                <div class="d-flex flex-column">
                                    <?php if($row->code_code): ?>
                                        <a href="<?= url('admin/code-update/' . $row->code_id) ?>"><?= $row->code_code ?></a>
                                    <?php else: ?>
                                        <span class="text-muted"><?= l('global.unknown') ?></span>
                                    <?php endif ?>

                                    <?php if($row->code_type): ?>
                                        <span class="text-muted small"><?= l('admin_codes.type_' . $row->code_type) ?></span>
                                    <?php endif ?>
                                </div>
                            </td>
                            ...
                            <td class="text-nowrap">
                                <span data-toggle="tooltip" data-html="true" title="<?= sprintf(l('global.datetime_tooltip'), '<br />' . \Altum\Date::get($row->datetime, 2) . '<br /><small>' . \Altum\Date::get($row->datetime, 3) . '</small>' . '<br /><small>(' . \Altum\Date::get_timeago($row->datetime) . ')</small>') ?>">
                                    <i class="fas fa-fw fa-calendar text-muted mr-1"></i><?= \Altum\Date::get($row->datetime, 1) ?>
                                </span>
                            </td>
                        </tr>
                    <?php endwhile ?>
                    ...
                    <tr>
                        <td colspan="4">
                            <a href="<?= url('admin/redeemed-codes') ?>" class="text-muted">
                                <i class="fas fa-angle-right fa-sm fa-fw mr-1"></i> <?= l('global.view_more') ?>
                            </a>
                        </td>
                    </tr>
...
<?php if($is_superadmin && in_array(settings()->license->type, ['SPECIAL', 'Extended License', 'extended'])): ?>
    ...
    <div class="col-12 col-sm-6 col-xl-4 p-3 position-relative">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body text-truncate">
                ...
                <div class="mt-2"><span class="h6">flowcode.co.ke</span></div>
            </div>

            <div class="pr-4 d-flex flex-column justify-content-center">
                <a href="https://flowcode.co.ke" class="stretched-link" target="_blank">
                    <i class="fas fa-fw fa-angle-right text-gray-500"></i>
                </a>
            </div>
        </div>
    ...
    <div class="col-12 col-sm-6 col-xl-4 p-3 position-relative">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body text-truncate">
                ...
                <div class="mt-2"><span class="h6">support@flowcode.co.ke</span></div>
            </div>
    ...
    <div class="col-12 col-sm-6 col-xl-4 p-3 position-relative">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body text-truncate">
                ...
                <div class="mt-2"><span class="h6">@flowcodeke</span></div>
            </div>
...
<script>
    (async function fetch_statistics() {
        ...
        if(data.status == 'error') {
            ...
            if(document.querySelector('#payments_total_amount')) {
            ...
            if(document.querySelector('#redeemed_codes')) {
                document.querySelector('#redeemed_codes').innerHTML = data.details.redeemed_codes ? nr(data.details.redeemed_codes) : 0;
                document.querySelector('#redeemed_codes_current_month').innerHTML = data.details.redeemed_codes_current_month ? nr(data.details.redeemed_codes_current_month) : 0;
            }
