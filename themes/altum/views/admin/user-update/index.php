...
if(!empty($data->subscription->current_period_start)) {
    try {
        $subscription_period_start = (new \DateTime($data->subscription->current_period_start))->format('Y-m-d\TH:i');
        ...
        <form action="" method="post" role="form" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select id="type" name="type" class="custom-select">
                    <option value="2" <?= $data->user->type == 2 ? 'selected="selected"' : null ?>>Superadmin</option>
                    <option value="1" <?= $data->user->type == 1 ? 'selected="selected"' : null ?>>Admin</option>
                    <option value="0" <?= $data->user->type == 0 ? 'selected="selected"' : null ?>>User</option>
                </select>
                <small class="form-text text-muted">Admins can access operational admin pages. Superadmins can access all admin pages and manage roles.</small>
            </div>
            ...
            <div id="subscription_period_start_container" class="form-group">
                <label for="subscription_period_start"><i class="fas fa-fw fa-sm fa-calendar-check text-muted mr-1"></i> <?= l('admin_user_update.plan.subscription_period_start') ?></label>
                <input id="subscription_period_start" type="datetime-local" name="subscription_period_start" class="form-control <?= \Altum\Alerts::has_field_errors('subscription_period_start') ? 'is-invalid' : null ?>" value="<?= $subscription_period_start ?>">
                <small class="form-text text-muted"><?= l('admin_user_update.plan.subscription_period_start_help') ?></small>
                <?= \Altum\Alerts::output_field_error('subscription_period_start') ?>
            </div>
...
<script>
    ...
    let check_plan_id = () => {
        ...
        if(selected_plan_id == 'free') {
            ...
            document.querySelector('#subscription_period_start_container').style.display = 'none';
        } else {
            document.querySelector('#plan_expiration_date_container').style.display = 'block';
            document.querySelector('#subscription_period_start_container').style.display = 'block';
        }
